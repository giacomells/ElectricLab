...
{
    // TFile ∗file = new TFile(”example.root”, ”RECREATE”); // Draw() −−−−−> DrawCopy()
    ...
    TF1 *f = new TF1("f", myFunction, -10, 10, 3); // 3 = #parameters f−>SetParameter(0, 1);
    f->SetParameter(1, 0);
    f->SetParameter(2, 1);
    ...
    std::cout << "Location of Max : " << h[0]->GetBinCenter(h[0]->GetMaximumBin()) << '\n';
